from moviepy import VideoFileClip, VideoClip, CompositeAudioClip, concatenate_videoclips, concatenate_audioclips
...
class Merger:
    ...
    def makeSubClips(self, timestamps_list: list) -> None:
        ...
        self.sub_clips = [self.full_clip.subclipped(timestamp['start'], timestamp['end']).audio
            for timestamp in timestamps_list]
        
    def write_concatenated_audio_clips(self):
        print(f"Merging sub-clips into audio and saving to {TMP}/merged.wav...\n")
        try:
            merged_audio: CompositeAudioClip = concatenate_audioclips(self.sub_clips)
